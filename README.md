...
|------|--------------|-----|------|-----------|--------|-----------------|
| CV（训练免） | 1.73% | 89.2% | 82.4% | 40% | 0 | ~800 |
| **U-Net（推荐）** | **0.77%** | 1.87% | 3.2% | **70%** | 0 | 788（245–1612） |

评估指标：`rel_rmse = RMSE / y 满量程`（GT 在预测 x 处线性插值），
`x_coverage` = 预测 x 范围与 GT 重叠比例。U-Net 后端中位数已满足
「≤1% 满量程」验收目标；CV 后端作为无训练后备，存在浅色/虚线曲线的已知
弱项。U-Net 单图推理（GPU）< 1 s，全流程瓶颈在 CPU OCR（~6 s）。
