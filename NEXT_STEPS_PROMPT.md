...
- **关键修复**：刻度像素改用 OCR 标签中心（CV mark 检测在 plot 顶部漂移 +9.5px）
- 剩余 1 张（img_0060 1.6%）：亚像素级残余，真实图阶段（B-5）再验证
...
### 5.1【无需训练】B-5a：OCR 刻度值误读增强（✅ 完成 2026-08-16：70%→99%）
- 已实施：A 候选解析+序列消歧（axis_kind.py）；B 低分/非数字文本过滤（tick_min_score、
  _is_tick_label）；C 4x 条带重 OCR；D 缺刻度补全 + **标签中心像素**（关键）；
  回归修复 4 轮（单调性/10N 平局偏好/3-tick 像素感知剔除/strict 关联+范围外保留/10px 去重）
- 未实施：CLAHE 预处理（收益未验证）、竖排 y 刻度旋转（平台数据无竖排，真实图阶段做）
